...
export async function toggleMealCompletion(token, planId, mealId, date, alternativeId = null) {
...
export async function completeDayMeals(token, planId, mealIds, date, activeAlts = {}) {
  if (!await validateToken(token)) {
    return { error: 'Unauthorized' }
  }
  ...
  const toInsert = mealIds
    ...
    .map(mealId => ({
      ...
      date: dateStr,
      completed_at: new Date().toISOString(),
      alternative_id: activeAlts[mealId] || null,
    }))
